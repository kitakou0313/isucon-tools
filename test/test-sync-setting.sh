...
do
    ssh -p ${FRONTEND_HOSTS_SSH_PORT[host_idx]} -i ${FRONTEND_HOSTS_SSH_PRIVATE_KEY[host_idx]} ${FRONTEND_HOSTS_SSH_USER[host_idx]}@${FRONTEND_HOSTS[host_idx]} 'cat /etc/nginx/.gitkeep' | diff -s - ./configs/nginx/.gitkeep
    if [ $? -eq 0 ]; then
...
do
    ssh -p ${DB_HOSTS_SSH_PORT[host_idx]} -i ${DB_HOSTS_SSH_PRIVATE_KEY[host_idx]} ${DB_HOSTS_SSH_USER[host_idx]}@${DB_HOSTS[host_idx]} 'cat /etc/mysql/.gitkeep' | diff -s - ./configs/mysql/.gitkeep
    if [ $? -eq 0 ]; then
