...
#   rsync -e "ssh -p ${APP_HOSTS_SSH_PORT[host_idx]} -i ${APP_HOSTS_SSH_PUB_KEY[host_idx]}" -av $(file path in local) ${APP_HOSTS_SSH_USER[host_idx]}@${APP_HOSTS[host_idx]}:$(file path in remote)
# done
