...
do
  ...
  -av ./configs/nginx/ ${FRONTEND_HOSTS_SSH_USER[host_idx]}@${FRONTEND_HOSTS[host_idx]}:/tmp/nginx
done
...
do
  ...
  -av ./configs/mysql/ ${DB_HOSTS_SSH_USER[host_idx]}@${DB_HOSTS[host_idx]}:/tmp/mysql
done
